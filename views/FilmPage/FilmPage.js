...
const SeeMoreButton = props => {
  const { classes, onToggle, expanded, selectedTab } = props

  return (
    <div className={classes.seeMoreOuter}>
      <a className={classes.seeMoreButton} onClick={() => onToggle()}>
        {!expanded && (
          <span className={classes.seeMoreCopy}>
            {selectedTab === "about" ? "see more" : "view order book"}
          </span>
        )}
        <FontAwesomeIcon
        ...
        {expanded && (
          <span className={classes.seeMoreCopy}>
            {selectedTab === "about" ? "see less" : "hide order book"}
          </span>
        )}
      </a>
    </div>
  ...
  render() {
    ...
    return (
      <>
        ...
        >
          {this.state.expanded && !store.userStore.token ? (
            <InvestNowSection />
          ) : (
            ""
          )}
