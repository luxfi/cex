  ...
  */
  ...
  @action addBalance(val, onSuccess, onError) {
    ...
    if (typeof parsedVal === 'number' && !isNaN(parsedVal)) {
      let oldBalance = localStorage.getItem('accountBalance')
  ...
  @action loadBalanceHistory() {
    ...
    this.balanceHistory = localStorage.getItem('balanceHistory')
  ...
  @action loadAccountBalance() {
    this.accountBalance = localStorage.getItem('accountBalance')
      ? Number.parseFloat(localStorage.getItem('accountBalance')).toFixed(2)
      : 0
  }

  @action handleFunds(vals, onSuccess, onError) {
    // Deposit or withdrawal funds to localStorage
    const { amount, deposit, fromAccount, toAccount, accountName, date } = vals
  ...
  @action async addPaymentMethod(token, meta, onSuccess, onError) {
    try {
      const { institution, accounts, account } = meta
      const opts = {
        publicToken: token,
  ...
  @action async addOfferingInvestment(amount, movieSlug, onSuccess, onError) {
    try {
      ...
      // Update history
      const offeringInvestments = localStorage.getItem('offeringInvestments')
        ? JSON.parse(localStorage.getItem('offeringInvestments'))
        : []
      offeringInvestments.unshift({ movieSlug, amount })
      this.offeringInvestments.unshift({ movieSlug, amount })
      localStorage.setItem(
        'offeringInvestments',
        JSON.stringify(offeringInvestments),
      )
      onSuccess && onSuccess()
    } catch (ex) {
      console.log('Error reducing funds from account', ex)
      onError && onError(ex.toString())
    }
  }

  @action loadOfferingInvestments() {
    // Loads the users balance from local
    this.offeringInvestments = localStorage.getItem('offeringInvestments')
      ? JSON.parse(localStorage.getItem('offeringInvestments'))
      : []
  }
  ...
  @computed get isValidPhotoIDs() {
    return [this.documents0, this.documents1, this.documents2].every(
      el => !isEmpty(el),
    )
  }
  ...
  @computed get formattedAccounts() {
    ...
    if (!this.account || !this.account.paymentMethods) return [esx]
    ...
    const accounts = this.account.paymentMethods.map(a => {
      return {
        ...
        account: toJS(
          a.Inputs.metadata.accounts[Math.floor(Math.random() * 4)],
        ),
        id: a.id,
      }
    })
